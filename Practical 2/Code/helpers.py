...
def drop_frequent_words(path_to_data, threshold, path_to_destination_file, lowercase=True):
    ##################################################################################################
    """Drops the most frequent (according to function dependent on threshold) words of the dataset."""
    ##################################################################################################
    ...
    for word in word_count:
        if (word in words_above_threshold):# TODO: Maybe just set it to 1?
            word_drop_out_prob[word] = (word_count[word] / (max_count * (1 + min_count/total_count_most_frequent_words)))**(1/10)# TODO: Find an appropriate value
        else:
    ...
    for pair in ordered_counts:
        print("Word: " + pair[0] + " ||| Count: " + str(word_count[pair[0]]) + " ||| Dropout prob: " + str(word_drop_out_prob[pair[0]]))
...
drop_frequent_words(path_to_data, 0.005, 0)
